        ...
        public FileMergeResult MergePAMEquityFinance(SqlConnection con, long pamId, string foldertemplate, string temporaryFolderLocation)
        {
			...
			try
			{
				...
				try
				{
					...
					app.ActiveDocument.Bookmarks["CxPROPOSALxExpectedHoldingPeriod"].Range.InsertFile(ConvertHtmlAndFile.SaveToHtml(dataResult[0].pricingCollateral));
					app.ActiveDocument.Bookmarks["CxPROPOSALxExitStrategy"].Range.InsertFile(ConvertHtmlAndFile.SaveToHtml(dataResult[0].pricingCollateral));
					app.ActiveDocument.Bookmarks["CxPROPOSALxExpectedReturn"].Range.InsertFile(ConvertHtmlAndFile.SaveToHtml(dataResult[0].pricingCollateral));					
					...
					this.FillBookmarkWithPAMAttachmentType1(app, con, "ShareValuationReport", AppConstants.TableName.PAM_ShareValuationReport, pamId);
